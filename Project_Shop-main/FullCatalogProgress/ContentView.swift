...
struct ProviderDetailSection: View {
    ...
    var body: some View {
        ...
        return VStack(alignment: .leading, spacing: 4) {
            ...
            // Shipping info
            if let shippingOptions = product.shippingInfoPerProvider?[provider.id] {
                Text("Shipping Options:")
                    .font(.subheadline)

                ForEach(shippingOptions, id: \.variantId) { option in
                    let handlingText = "\(option.handlingTime.value) \(option.handlingTime.unit)"
                    let firstCostText = "\(option.firstItemCost.cost) \(option.firstItemCost.currency)"
                    let additionalCostText = "\(option.additionalItemCost.cost) \(option.additionalItemCost.currency)"
                    let countryList = option.countries.joined(separator: ", ")
                    VStack(alignment: .leading, spacing: 4) {
                        Text("Variant ID: \(option.variantId)")
                        Text("Handling Time: \(handlingText)")
                        Text("First Item Cost: \(firstCostText)")
                        Text("Additional Item Cost: \(additionalCostText)")
                        Text("Countries: \(countryList)")
                    }
                    Divider()
                }
            }
