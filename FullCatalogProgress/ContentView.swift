            ...
            if let providerVariants = product.variantImagesAvailability?[provider.id],
               let shippingOptions = product.shippingInfoPerProvider?[provider.id] {
                ...
                ForEach(providerVariants) { variant in
                    ...
                    // Raw Variant JSON
                    if let varData = try? JSONEncoder().encode(variant),
                       let varJSON = String(data: varData, encoding: .utf8) {
                        ScrollView(.horizontal) {
                            Text(varJSON)
                                .font(.caption)
                                .lineLimit(nil)
                    ...
                    if let optionsForVariant = groupedByVariant[variantId],
                       let jsonData = try? JSONEncoder().encode(optionsForVariant),
                       let jsonString = String(data: jsonData, encoding: .utf8) {
                        ScrollView(.horizontal) {
                            Text(jsonString)
                                .font(.caption)
